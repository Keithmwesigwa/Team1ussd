...
export default function CitizenPortal({ onRefresh, authSession, onLogout }: { onRefresh: () => void; authSession?: any; onLogout?: () => void }) {
  ...
  // Subscriber context
  const [citizenPhone, setCitizenPhone] = useState('+256772123456'); 
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setLoginSuccessMsg('Verification code sent! (Use code 123456 to verify)');
      } else {
        const err = await res.json();
        setLoginErrorMsg(err.error || 'Failed to request code.');
      }
    } catch (err) {
      setLoginOtpSent(true);
      setLoginSuccessMsg('[Offline Fallback] Code simulated! Enter 123456.');
    } finally {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        localStorage.setItem('tulinde_subscriber_phone', loginPhone);
        localStorage.setItem('tulinde_citizen_token', data.token);
        setCitizenPhone(loginPhone);
        setIsCitizenAuth(true);
        ...
        const err = await res.json();
        setLoginErrorMsg(err.error || 'Invalid verification code.');
      }
    } catch (err) {
      if (loginOtpCode === '123456') {
        localStorage.setItem('tulinde_subscriber_phone', loginPhone);
        localStorage.setItem('tulinde_citizen_token', 'mock-offline-token');
        setCitizenPhone(loginPhone);
        setIsCitizenAuth(true);
        setLoginOtpSent(false);
        setLoginOtpCode('');
        ...
        setLoginErrorMsg('Invalid verification code.');
      }
    } finally {
      setLoginLoading(false);
    }
    ...
    <div className="flex-1 w-full bg-[#0A0A0C] text-gray-200 py-6 px-4 md:px-8">
      ...
      <div className="max-w-7xl mx-auto flex flex-col items-center">
        ...
        {viewMode === 'mobile' ? (
          ...
          <div className="md:relative md:w-[375px] md:h-[780px] bg-[#0E1116] md:rounded-[52px] md:border-[14px] md:border-[#22252A] md:shadow-2xl overflow-hidden flex flex-col justify-between w-full min-h-[700px] md:min-h-0 rounded-none border-none shadow-none">
            {/* iPhone screen notches */}
            <div className="hidden md:flex absolute top-0 left-1/2 transform -translate-x-1/2 w-40 h-7 bg-[#22252A] rounded-b-2xl z-50 items-center justify-between px-6">
            ...
            <div className="flex-1 flex flex-col bg-[#0F172A] overflow-y-auto px-4 pt-4 md:pt-10 pb-4">
              {!isCitizenAuth ? (
                <div className="flex-1 flex flex-col justify-center space-y-6 px-2 my-auto">
                  <div className="text-center">
                    <h3 className="text-xl font-bold text-white tracking-tight">Tulinde PWA Gate</h3>
                    <p className="text-[10px] uppercase tracking-wider font-extrabold text-[#D97706] mt-1">Consumer Registry Security Gateway</p>
                  </div>

                  {!loginOtpSent ? (
                    <form onSubmit={handleSendOtp} className="space-y-4">
                      <div>
                        <label className="block text-[9px] uppercase font-bold text-slate-400 mb-1.5">Mobile Money Phone Number</label>
                        <input 
                          type="text" 
                          required
                          value={loginPhone}
                      ...
                      {loginErrorMsg && (
                        <div className="p-3 bg-red-500/10 border border-red-500/25 rounded-xl text-red-400 text-[10px] font-bold flex items-center gap-1.5">
                      ...
                      <button 
                        ...
                        className="w-full bg-[#2563EB] hover:bg-[#2563EB]/90 py-2.5 rounded-xl text-white font-bold text-xs uppercase tracking-wider transition cursor-pointer"
                      >
                        {loginLoading ? 'Requesting OTP...' : 'Request Verification Code'}
                      </button>
                    </form>
                  ) : (
                    <form onSubmit={handleVerifyOtp} className="space-y-4">
                      <div>
                        <label className="block text-[9px] uppercase font-bold text-slate-400 mb-1.5">Verification Code (OTP)</label>
                        <input 
                          type="text" 
                          required
                          value={loginOtpCode}
                      ...
                      {loginSuccessMsg && (
                        <div className="p-3 bg-emerald-500/10 border border-emerald-500/25 rounded-xl text-emerald-400 text-[10px] font-bold flex items-center gap-1.5">
                      ...
                      <button 
                        type="submit"
                        disabled={loginLoading}
                        className="w-full bg-emerald-600 hover:bg-emerald-500 py-2.5 rounded-xl text-white font-bold text-xs uppercase tracking-wider transition cursor-pointer"
                      >
                  ...
                  <div className="bg-slate-950/80 border border-slate-900 rounded-xl p-3 text-[10px] font-mono text-slate-400 text-center leading-relaxed">
                    <span className="text-white block font-bold mb-1">Developer Test Access:</span>
                  ...
                  <div className="flex items-center justify-between mt-3 mb-1">
                    ...
                    <div className="flex gap-1 items-center">
                      ...
                      <button 
                        ...
                        className="ml-1 bg-red-500/10 border border-red-500/20 text-[9px] font-black text-red-400 px-2 py-0.5 rounded uppercase tracking-wider hover:bg-red-500 hover:text-white transition cursor-pointer"
                      >
                        Logout
                      </button>
                    </div>
                  ...
                  {activeTab === 'report' && (
                    <div className="flex-1 flex flex-col justify-between">
                      ...
                      <form onSubmit={handleComplaintSubmit} className="space-y-3.5">
                        {/* Category */}
                        <div>
                          <label className="block text-[9px] uppercase font-bold text-slate-400 mb-1.5">{t.categoryLabel}</label>
                          <select 
                            value={formCategory} 
                        ...
                        {/* Operator */}
                        <div>
                          <label className="block text-[9px] uppercase font-bold text-slate-400 mb-1.5">{t.providerLabel}</label>
                          <select 
                            value={formProvider} 
                        ...
                        {/* Txn ID */}
                        <div>
                          <label className="block text-[9px] uppercase font-bold text-slate-400 mb-1.5">{t.txnLabel}</label>
                          <input 
                            type="text" 
                            value={formTxnId}
                  ...
                  {activeTab === 'verify' && (
                    <div className="flex-1 flex flex-col justify-between">
                      ...
                      <form onSubmit={handleVerifyNumber} className="space-y-4">
                        <div>
                          <label className="block text-[9px] uppercase font-bold text-slate-400 mb-1 animate-fade-in">Verify Mobile Wallet Number</label>
                          <input 
                            type="text"
                            required 
                            value={verifyPhone}
          ...
          <div className="w-full bg-[#0E1116] rounded-3xl border border-slate-800 shadow-2xl p-6">
            {!isCitizenAuth ? (
              <div className="max-w-md mx-auto py-12 space-y-6">
                <div className="text-center">
                  <h3 className="text-2xl font-bold text-white tracking-tight">Tulinde PWA Gate</h3>
                  <p className="text-xs uppercase tracking-wider font-extrabold text-[#D97706] mt-1.5">Consumer Registry Security Gateway</p>
                </div>

                {!loginOtpSent ? (
                  <form onSubmit={handleSendOtp} className="space-y-4">
                    <div>
                      <label className="block text-[10px] uppercase font-bold text-slate-400 mb-1.5">Mobile Money Phone Number</label>
                      <input 
                        type="text" 
                        required
                        value={loginPhone}
                    ...
                    <button 
                      type="submit"
                      disabled={loginLoading}
                      className="w-full bg-[#2563EB] hover:bg-[#2563EB]/90 py-3 rounded-xl text-white font-bold text-xs uppercase tracking-wider transition cursor-pointer"
                    >
                      {loginLoading ? 'Requesting OTP...' : 'Request Verification Code'}
                    </button>
                  </form>
                  ...
                  <form onSubmit={handleVerifyOtp} className="space-y-4">
                    ...
                    <button 
                      type="submit"
                      disabled={loginLoading}
                      className="w-full bg-emerald-600 hover:bg-emerald-500 py-3 rounded-xl text-white font-bold text-xs uppercase tracking-wider transition cursor-pointer"
                    >
                      {loginLoading ? 'Verifying OTP...' : 'Verify & Sign In'}
                    </button>
                    ...
                    <button 
                      type="button"
                      onClick={() => setLoginOtpSent(false)}
                      className="w-full text-slate-400 hover:text-white text-xs font-bold transition text-center block"
                    >
                      ← Change Phone Number
                    </button>
                  </form>
                ...
                <div className="bg-slate-950/80 border border-slate-900 rounded-xl p-4 text-xs font-mono text-slate-400 text-center leading-relaxed">
                  <span className="text-white block font-bold mb-1">Developer Test Access:</span>
                  Enter any phone number and use verification code: <strong className="text-emerald-400">123456</strong>
                </div>
              </div>
